...
from objectSelection import geometricMatch, isGoodLepton, SingleMuonVeto, SingleElectronVeto, SingleLepVeto, TwoMuonVeto
from plottingTools import plotDataVSMC
from helpers import getObjFromFile, progress
import Sample
...
argParser.add_argument('--sampleName',  action='store',         default='DYJetsToLL_M-10to50')
argParser.add_argument('--subJob',      action='store',         default=None)
argParser.add_argument('--isTest',      action='store',         default=False)

args = argParser.parse_args()

sampleList = Sample.createSampleList('/user/lwezenbe/private/PhD/Code/TauStudy/ReproduceAN2017_094/Data/inputFiles.conf')
...
for entry in eventRange:
    ...
    if not sample.isData :
        weightfactor = getWeightFactor(sample.xsec, Chain._weight, sample.hCount)
    else :
        weightfactor = 1
...
if not args.isTest:
    for var in xrange(cst.NumberOfVar):
        listOfHist[var].SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/ReproduceAN2017_094/Histos/" + sample.output + "/" + cst.varInfo[var][0] + "_Hist_" + args.sampleName+"_"+args.subJob + ".root")
