...
void SetBranches(TString FileName, Bool_t isSignal){
   ...
   if(isSignal){
       Tree->SetBranchAddress("_mChi1", &_mChi1, &b__mChi1);
       Tree->SetBranchAddress("_mChi2", &_mChi2, &b__mChi2);
   }
}
...
void ProduceROCPlots(){
    ...
    for(int sample = 0; sample < NumberOfSamples; ++sample){                               //Run over all samples and calculate the efficiency
        ...
        SetBranches(FileName[sample], isSignal);
        nPromptTaus = 0;
        ...
        for(Long64_t entry = 0; entry < nentry; entry++){                               //Loop over all entries
            ...
            // if(isSignal and (_mChi1 != m1 or _mChi2 != m2))             continue;
            if(isSignal and (_mChi2-_mChi1 != m2-m1))             continue;
        ...
        for(int discriminator = 0; discriminator < NumberOfDiscr; discriminator++){
            for(int WP = 0; WP < NumberOfWP; ++WP){
                EfficiencyErrors[sample][discriminator][WP] = (sqrt(Efficiency[sample][discriminator][WP])/nPromptTaus)*100;
    ...
    ROCmvaOld->Draw("AP");
    CMS_lumi(c1, 4, 11);
    c1->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_MVAOld.pdf");
    c1->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_MVAOld.jpg");
    ...
    ROCmvaNew->Draw("AP");
    CMS_lumi(c2, 4, 11);
    c2->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_MVANew.pdf");
    c2->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_MVANew.jpg");
    ...
    ROCCuts->Draw("AP");
    CMS_lumi(c3, 4, 11);
    c3->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_Cuts.pdf");
    c3->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/ROC/"+FileName[Signal]+"/ROC_Cuts.jpg");
