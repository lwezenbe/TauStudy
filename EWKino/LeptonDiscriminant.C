...
void LeptonDiscriminant(){
    ...
    TH1D* EfficiencyHist = new TH1D("EfficiencyMVA", "EfficiencyMVA", 3, 0, 3);         //Define Hist that will show the efficiency of different discriminator
    TH1D* EfficiencyPtHist[NLeptonDiscr][NumberOfFracComp];
    TH1D* EfficiencyEtaHist[NLeptonDiscr][NumberOfFracComp];

    TString histtitle;

    for(int discriminator = 0; discriminator < NLeptonDiscr; discriminator++){                                                         //Initialize the enumerator and denominator for the calculation of efficiency
    ...
    for(Long64_t entry = 0; entry < 3000000; entry++){                               //Loop over all entries
        progress = Progressbar(progress, entry, 3000000);                            //Increase progress in progressbar
        Tree->GetEntry(entry);
        ...
        for(int lepton = 0; lepton < _nL; lepton++){                                //Loop over all leptons in event
            ...
            nPromptTaus++; 
            for(int discriminator = 0; discriminator < NLeptonDiscr; discriminator++){
            ...
            for(int discriminator = 0; discriminator < NLeptonDiscr; discriminator++){
                if(discriminator == EleDiscr and _tauEleVeto[lepton]){                                                  EfficiencyHist->AddBinContent(1); EfficiencyPtHist[discriminator][Enumerator]->Fill(_lPt[lepton]); EfficiencyEtaHist[discriminator][Enumerator]->Fill(_lEta[lepton]);}                 //Old tau ID
                if(discriminator == MuDiscr and _tauMuonVeto[lepton]){                                                  EfficiencyHist->AddBinContent(2); EfficiencyPtHist[discriminator][Enumerator]->Fill(_lPt[lepton]); EfficiencyEtaHist[discriminator][Enumerator]->Fill(_lEta[lepton]);}
                if(discriminator == BothLepDiscr and _tauEleVeto[lepton] and _tauMuonVeto[lepton]){                    EfficiencyHist->AddBinContent(3); EfficiencyPtHist[discriminator][Enumerator]->Fill(_lPt[lepton]); EfficiencyEtaHist[discriminator][Enumerator]->Fill(_lEta[lepton]);}
            }
    ...
    EfficiencyHist->Scale(100./nPromptTaus);
    for(int discriminator = 0; discriminator < NLeptonDiscr; discriminator++){
...
//Draw
    ...
    EfficiencyHist->GetXaxis()->SetBinLabel(1, "e discr");
    EfficiencyHist->GetXaxis()->SetBinLabel(2, "#mu discr");
    EfficiencyHist->GetXaxis()->SetBinLabel(3, "both discr");
    EfficiencyHist->SetTitle("Efficiency of different lepton discriminators against electrons and muons; ; Efficiency (%)");
    EfficiencyHist->SetLineColor(kBlack);
    EfficiencyHist->GetYaxis()->SetTitleOffset(1.1);
    EfficiencyHist->SetFillColor(kGray);
    EfficiencyHist->SetMinimum(0);
    EfficiencyHist->SetMaximum(100);
    EfficiencyHist->Draw("EHISTTEXT");
    // CMS_lumi(c1, 4, 1);
    c1->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/"+FileName+"/TauEwkinoLeptonDiscr.pdf");
    delete c1;
    delete EfficiencyHist;

    TCanvas * c2 = new TCanvas("c2, c2");
    EfficiencyPtHist[EleDiscr][Enumerator]->SetLineColor(kBlack);
    EfficiencyPtHist[EleDiscr][Enumerator]->GetXaxis()->SetNdivisions(508);
    EfficiencyPtHist[EleDiscr][Enumerator]->SetMinimum(0);
    EfficiencyPtHist[EleDiscr][Enumerator]->SetMaximum(100);
    ...
    EfficiencyPtHist[EleDiscr][Enumerator]->SetTitle(plottitle);
    EfficiencyPtHist[EleDiscr][Enumerator]->Draw("HIST");
    EfficiencyPtHist[MuDiscr][Enumerator]->SetLineColor(kBlue);
    EfficiencyPtHist[MuDiscr][Enumerator]->Draw("HISTSAME");
    EfficiencyPtHist[BothLepDiscr][Enumerator]->SetLineColor(kRed);
    EfficiencyPtHist[BothLepDiscr][Enumerator]->Draw("HISTSAME");
    auto legend = new TLegend(.85,.7,1., .9);
    legend->AddEntry(EfficiencyPtHist[EleDiscr][Enumerator], "e discr");
    legend->AddEntry(EfficiencyPtHist[MuDiscr][Enumerator], "#mu discr");
    legend->AddEntry(EfficiencyPtHist[BothLepDiscr][Enumerator], "both");
    legend->SetFillStyle(0);
    legend->SetBorderSize(0);
    legend->Draw();
    CMS_lumi(c2, 4, 11);
    c2->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/"+FileName+"/TauEwkinoLeptonDiscrAsFuncOfPT.pdf");
    delete c2;

    TCanvas * c3 = new TCanvas("c3, c3");
    EfficiencyEtaHist[EleDiscr][Enumerator]->SetLineColor(kBlack);
    EfficiencyEtaHist[EleDiscr][Enumerator]->GetXaxis()->SetNdivisions(508);
    EfficiencyEtaHist[EleDiscr][Enumerator]->SetMinimum(0);
    EfficiencyEtaHist[EleDiscr][Enumerator]->SetMaximum(100);
    // plottitle = "Efficiency of different new discriminators as a function of the transverse momentum of the lepton; p_{T}(#tau) [GeV]; Efficiency (%) / " + to_string_with_precision(EfficiencyEtaHist[VLoose_Old][Enumerator]->GetBinWidth(1), 2) + "GeV";
    plottitle = "; p_{T}(#tau) [GeV]; Efficiency (%) / " + to_string_with_precision(EfficiencyEtaHist[VLoose_Old][Enumerator]->GetBinWidth(1), 2) + "GeV";
    EfficiencyEtaHist[EleDiscr][Enumerator]->SetTitle(plottitle);
    EfficiencyEtaHist[EleDiscr][Enumerator]->Draw("HIST");
    EfficiencyEtaHist[MuDiscr][Enumerator]->SetLineColor(kBlue);
    EfficiencyEtaHist[MuDiscr][Enumerator]->Draw("HISTSAME");
    EfficiencyEtaHist[BothLepDiscr][Enumerator]->SetLineColor(kRed);
    EfficiencyEtaHist[BothLepDiscr][Enumerator]->Draw("HISTSAME");

    auto legend1 = new TLegend(.85,.7,1., .9);
    legend1->AddEntry(EfficiencyEtaHist[EleDiscr][Enumerator], "e discr");
    legend1->AddEntry(EfficiencyEtaHist[MuDiscr][Enumerator], "#mu discr");
    legend1->AddEntry(EfficiencyEtaHist[BothLepDiscr][Enumerator], "both");

    legend1->SetFillStyle(0);
    legend1->SetBorderSize(0);
    legend1->Draw();
    CMS_lumi(c3, 4, 11);
    c3->SaveAs("/user/lwezenbe/private/PhD/Results/TauStudy/EWKino/"+FileName+"/TauEwkinoLeptonDiscrAsFuncOfEta.pdf");
    delete c3;
