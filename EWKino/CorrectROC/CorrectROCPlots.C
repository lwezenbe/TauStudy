...
void CorrectROCPlots(){
    ...
    TH1D * IsoHist[2];
    IsoHist[Signal] = TH1D("IsoMVAold", "IsoMVAold", 20, -1, 1);
    IsoHist[Signal]->sumw2();
    IsoHist[Background] = TH1D("IsoMVAold", "IsoMVAold", 20, -1, 1);
    IsoHist[Background]->sumw2();

    // TString FileName = "WZTo3LNu";
    //~ TString FileName = "displacedTTJets_skimmedwcopytree";
    TString FileName = "TTJetsOldDM";
    ...
    for(Long64_t entry = 0; entry < nentry; entry++){                               //Loop over all entries
        ...
        for(int lepton = 0; lepton < _nL; lepton++){                                //Loop over all leptons in event
            ...
            IsoHist[sample]->Fill(_lPOGMedium[lepton]);
                
            nPromptTaus[sample]++;

            // cout << _decayModeFinding[lepton] << " " << _decayModeFindingNew[lepton] << endl;
            
            if(!_decayModeFindingNew[lepton])                   continue;                       //New tau decay mode finding
            if(!_decayModeFinding[lepton]){++test2;}

            // if(!_decayModeFinding[lepton])                      continue;                       //Old tau decay mode finding
            ...
            if(_lPOGMedium[lepton]){                            ++Efficiency[sample][MVA_Old][Medium_MVA];     if(!_decayModeFinding[lepton]){++test;}}           
    ...
    cout <<nPromptTaus[Background] << " " << nPromptTaus[Signal] << " " << Efficiency[Background][MVA_Old][Medium_MVA] << " " << Efficiency[Signal][MVA_Old][Medium_MVA] << " " << test << " " << test2 << endl;
    ...
    cout << Efficiency[Background][MVA_Old][Medium_MVA] << " " << Efficiency[Signal][MVA_Old][Medium_MVA] << endl;
    ...
    TCanvas * IsoHistCanv = new TCanvas("IsoHistCanv", "IsoHistCanv", 500, 500);
    IsoHist[Signal]->SetLineColor(kRed);
    IsoHist[Signal]->Draw("EHist");
    IsoHist[Background]->Draw("EHistSame");
    TLegend IsoLeg = new TLegend(
